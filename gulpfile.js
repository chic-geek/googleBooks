...
var
  settings = {
    ...
    // if you change the 'source' and want to use AngularJS then you'll need to amend that also.
    dir: {
      source: 'source/',
      public: 'public/',
      dist:   'build/',
...
gulp.task('watch', function() {
  ...
  gulp.watch( settings.angular.dir + '/**/*', ['ngApp-build']);
});
...
gulp.task('ngApp-build', function() {
  if (settings.angular.app) {
    return gulp.src(
      settings.angular.dir + '**/*', { base: './' + settings.dir.source }
    )
    .pipe(gulp.dest( settings.dir.public ));
  }
});
...
gulp.task('pipeline', ['styles', 'scripts', 'images', 'fonts', 'bower-install', 'ngApp-build'], function() {
  gulp.start('watch');
});
...
gulp.task('pipeline-build', ['spring-clean', 'styles', 'scripts', 'images', 'fonts', 'bower-install', 'ngApp-build', 'fileinclude'], function() {
